...
params:
  studentID: "410573076"
  studentName: "陳盈蓁"
  repoURL: "https://github.com/Chen-Ying-Chen/inclassPractice1/blob/master/Exercise4-410573076.Rmd"
---
...
```{r ans12}
library(stringr)
str_which(colnames(gsHW2ws),"ans")
```
...
```{r ans16}
correct4 <- gsHW2ws[gsHW2ws$ans23==1,] 
b <- colSums(correct4[,1:5],na.rm=TRUE)
which(b == 9)
```
...
```{r ans17}
correct3 <- gsHW2ws[gsHW2ws$ans22==1,]
a <- colMeans(correct3[,1:5],na.rm=TRUE)
which(a <= 0.5)
```
...
ymd(allBankData$年月) -> allBankData$年月
allBankData$年月 + years(911) -> allBankData$年月
```
...
allBankData %>% 
  mutate(
   `銀行`=as.factor(`銀行`),
   `年月`=as.Date(`年月`),
  `定存利率-三個月-固定`=as.numeric(`定存利率-一年期-固定`),
...
exData %>% 
  mutate(
   `期間`=as.character(`期間`),
  `新台幣NTD/USD`=as.numeric(`新台幣NTD/USD`),
  `日圓JPY/USD`=as.numeric(`日圓JPY/USD`),
  `英鎊USD/GBP`=as.numeric(`英鎊USD/GBP`),
  `港幣HKD/USD`=as.numeric(`港幣HKD/USD`),
  `韓元KRW/USD`=as.numeric(`韓元KRW/USD`),
  `加拿大幣CAD/USD`=as.numeric(`加拿大幣CAD/USD`),
  `新加坡元SGD/USD`=as.numeric(`新加坡元SGD/USD`),
  `人民幣CNY/USD`=as.numeric(`人民幣CNY/USD`),
  `澳幣USD/AUD`=as.numeric(`澳幣USD/AUD`),
  `泰銖THB/USD`=as.numeric(`泰銖THB/USD`),
  `馬來西亞幣MYR/USD`=as.numeric(`馬來西亞幣MYR/USD`),
  `菲律賓披索PHP/USD`=as.numeric(`菲律賓披索PHP/USD`),
  `歐元USD/EUR`=as.numeric(`歐元USD/EUR`),
  `馬克DEM/USD`=as.numeric(`馬克DEM/USD`),
  `法國法郎FRF/USD`=as.numeric(`法國法郎FRF/USD`),
  `荷蘭幣NLG/USD`=as.numeric(`荷蘭幣NLG/USD`),
  `越南盾VND/USD`=as.numeric(`越南盾VND/USD`)
  ) -> exData
...
```{r ans32}
exData %>% 
  mutate(`英鎊USD/GBP`=1/`英鎊USD/GBP`,
         `澳幣USD/AUD`=1/`澳幣USD/AUD`,
         `歐元USD/EUR`=1/`歐元USD/EUR`
         ) -> exData
exData
```
...
```{r ans33}
colnames(exData) <- c("期間","新台幣NTD/USD","日圓JPY/USD","英鎊","港幣HKD/USD","韓元KRW/USD","加拿大幣CAD/USD","新加坡元SGD/USD","人民幣CNY/USD","澳幣","泰銖THB/USD","馬來西亞幣MYR/USD","菲律賓披索PHP/USD","歐元","馬克DEM/USD","法國法郎FRF/USD","荷蘭幣NLG/USD","越南盾VND/USD")
exData
```

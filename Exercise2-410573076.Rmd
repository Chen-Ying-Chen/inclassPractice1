...
```{r ans1}
library(stringr)
as.numeric(str_extract(str_replace(iPeenData$均消,",",""), "\\d+"))
```
...
```{r ans21}
library(lubridate)
POSIXcttime <- mdy_hms(filmVotingData$Timestamp)
```
...
```{r ans22}
library(lubridate)
Taipeitime <- with_tz(POSIXcttime, "Asia/Taipei")
```
...
```{r ans23}
library(lubridate)
hour(Taipeitime) -> filmVotingData$hour 
```

2.4 使用`table()`統計學生最活躍的填表時間。

```{r ans24}
as.data.frame(table(filmVotingData$hour))
```
